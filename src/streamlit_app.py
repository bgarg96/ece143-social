...
from config import FILTERS, PRIMARY_KEY, TOP_N, MONTHS, METRICS, PLATFORMS
...
if __name__ == '__main__':
    ...
    my_expander1 = st.expander(label='Show Top Influencers across all Categories')
    with my_expander1:
...
=======
    ...
    with my_expander3:
        optionC = st.selectbox('Choose a Country', platform.get_category_items(FILTERS[0]))
        # list of N's for top influencers
        # default value of N to be displayed in selection box
        ...
        df_cat=platform.get_N_most_popular_cat(platform.filter_by_month(platform.df,option_months),optionM,optionN,optionC)
        ...
        plt.rcdefaults()
        fig, ax = plt.subplots()
        ax.barh(df_cat[FILTERS[1]], df_cat['Aggregated '+optionM], align='center')
        ax.invert_yaxis()  # labels read top-to-bottom
    ...
    with my_expander3:
        optionP = st.selectbox('Choose a Category', platform.get_category_items(FILTERS[1]))
        # list of N's for top influencers
        # default value of N to be displayed in selection box
        optionsN2= [1,3,5,10,"All"]
        default_topn=TOP_N.index(3)
        optionN2 = st.selectbox(
            'Select the top N categories to display',optionsN2,index=default_topn)
        optionM2 = st.selectbox('Select an Aggregated Metric', platform.metrics)
        df_cat=platform.get_N_most_popular_country(platform.filter_by_month(platform.df,option_months),optionM2,optionN2,optionP)
        ...
        plt.rcdefaults()
        fig, ax = plt.subplots()
        ax.barh(df_cat[FILTERS[0]], df_cat['Aggregated '+optionM2], align='center')
        ax.invert_yaxis()  # labels read top-to-bottom
